...
var (
	...
	keyPass        = flag.String("keyPass", "testpswd", "key password")
...
func main() {
	...
	if *tpmPrivateKey != "" {
		...
		// go-tpm-keyfiles also provides a signer but here w're just using mine
		//k.Signer()

		// go-tpm-tools also provides a signer but yeah, we're still using mine
		// https://pkg.go.dev/github.com/google/go-tpm-tools/client#Key.GetSigner
